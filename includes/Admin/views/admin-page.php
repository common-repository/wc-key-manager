...
$tabs        = apply_filters( 'wc_key_manager_' . $page_id . '_tabs', $tabs );
	...
	<div class="wrap bk-wrap woocommerce">
		<?php if ( ! empty( $tabs ) && count( $tabs ) > 1 ) : ?>
			<nav class="nav-tab-wrapper bk-navbar">
				 ...
				 * @since 1.0.0
				 */
				do_action( 'wc_key_manager_' . $page_id . '_nav_items', $current_tab, $tabs );
				?>
			</nav>
		<?php endif; ?>
		<?php
		if ( ! empty( $current_tab ) && $page_id !== $current_tab ) {
			 ...
			 * @since 1.0.0
			 */
			do_action( "wc_key_manager_{$page_id}_{$current_tab}_content", $current_tab );
		} else {
			/**
			 ...
			 * @since 1.0.0
			 */
			do_action( "wc_key_manager_{$page_id}_content", $current_tab );
		}
		?>
	</div>
<?php
